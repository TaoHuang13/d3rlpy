...
class EnsSACImpl(DDPGBaseImpl):
    ...
    def _build_prior_weight(self) -> None:
        self._prior_weight = MLP(self._observation_shape[0], len(self._prior) + 1)

    def _build_prior_weight_optim(self) -> None:
        self._prior_weight_optim = self._temp_optim_factory.create(
            self._prior_weight.parameters(), lr=5e-6
        )
    ...
    def compute_actor_loss(self, batch: TorchMiniBatch) -> torch.Tensor:
        ...
        # kl_div = self.mc_kl_divergence(policy_dist, prior_dist, prior_weight) * self._log_temp().exp()

        # q_t = self._q_func(batch.observations, action, "min")
        # return (kl_div - q_t).mean()

        '''Log mixed prob'''
        # prior_log_prob = self._prior[0].compute_log_prob(batch.observations, action).detach()
        # prior_prob = prior_log_prob.exp()

        # prior_log_prob1 = self._prior[1].compute_log_prob(batch.observations, action).detach()
        # prior_prob1 = prior_log_prob1.exp()

        # uniform_prob =  torch.ones_like(prior_log_prob) * 0.5   # assume [-1, 1] action range

        # prior_weight = self._prior_weight(batch.observations).detach()
        # mixed_prob = prior_weight[:, 0].unsqueeze(1) * prior_prob + prior_weight[:, 1].unsqueeze(1) * prior_prob1 + prior_weight[:, 2].unsqueeze(1) * uniform_prob + 1e-10#(1 - prior_weight) * uniform_prob + 1e-10
        ...
        # uniform_prob =  torch.ones_like(prior_log_prob) * 0.5   # assume [-1, 1] action range
        # uniform_prob = torch.log(uniform_prob)

        # prior_weight = self._prior_weight(batch.observations).detach()
        # mixed_prob = prior_weight[:, 0].unsqueeze(1) * prior_log_prob + prior_weight[:, 1].unsqueeze(1) * prior_log_prob1 + prior_weight[:, 2].unsqueeze(1) * uniform_prob #+ 1e-10 #(1 - prior_weight) * uniform_prob + 1e-10
        # mixed_log_prob = mixed_prob * self._log_temp().exp() 

        '''KL div'''

        prior_weight = self._prior_weight(batch.observations).detach()
        kl_div = self.mc_kl_divergence(batch.observations, prior_weight, n_samples=5) * self._log_temp().exp()

        q_t = self._q_func(batch.observations, action, "min")
        return -(q_t - kl_div).mean()

        #return (entropy - q_t - mixed_log_prob).mean()

    def mc_kl_divergence(self, obs, prior_weight, n_samples=1):
        policy_dist = self._policy.dist(obs)
        prior_dist = []
        for i in range(len(self._prior)):
            prior_dist.append(self._prior[i].dist(obs))
        ...
        for x in samples:
            ...
            # prior_log_prob1 = prior_dist1.log_prob(x)
            # prior_prob1 = prior_log_prob1.exp()

            # uniform_prob = torch.ones_like(prior_log_prob) * 0.5

            # mixed_prob = prior_weight[:, 0].unsqueeze(1) * prior_prob + prior_weight[:, 1].unsqueeze(1) * prior_prob1 + prior_weight[:, 2].unsqueeze(1) * uniform_prob + 1e-10
            ...
            prior_log_probs = torch.stack(prior_log_probs, dim=1).squeeze(-1) # batch * ens * 1
            mixed_log_prob = (prior_weight * prior_log_probs).sum(1).unsqueeze(-1)

            kl_div.append(policy_log_prob - mixed_log_prob)
        print(policy_log_prob.mean(), prior_log_probs.mean(0))
        #print(policy_log_prob.mean(), mixed_log_prob.mean())
    ...
    def compute_prior_weight_loss(self, batch: TorchMiniBatch) -> torch.Tensor:
        ...
        q_mixes = []
        for i in range(len(self._prior)):
            prior_action = self._prior[i](batch.observations, deterministic=False, with_log_prob=False).detach()
            q_prior = self._q_func(batch.observations, prior_action, "min") 
            q_mixes.append(q_prior)

        uniform_action = torch.rand(prior_action.size()) * 2 - 1
        q_unif = self._q_func(batch.observations, uniform_action, "min")
        q_mixes.append(q_unif)
    ...
    def update_temp(
        ...
        with torch.no_grad():
            prior_weight = self._prior_weight(batch.observations).detach()
            kl_div = self.mc_kl_divergence(batch.observations, prior_weight, n_samples=5)
    ...
    def compute_target(self, batch: TorchMiniBatch) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            '''Mixed log prob'''
            # prior_log_prob = self._prior[0].compute_log_prob(batch.next_observations, action).detach()

            # prior_log_prob1 = self._prior[1].compute_log_prob(batch.next_observations, action).detach()
            ...
            '''KL div'''
            prior_weight = self._prior_weight(batch.next_observations).detach()
            kl_div = self.mc_kl_divergence(batch.next_observations, prior_weight, n_samples=5)

            return target - kl_div * self._log_temp().exp()
    ...
    #     with torch.no_grad():
    #         for i in range(len(self._prior)):
    #             action = self._prior[i].sample(x)
    #             actions.append(action)

    #             q, q_std = self._q_func.std(x, action)
    #             q_ucb = q + 10 * q_std
    #             qs.append(q_ucb)
...
class MLP(nn.Module):
    def __init__(self, in_dim, out_dim, hidden_size=256) -> None:
        ...
        self.mlp = nn.Sequential(
            ...
            nn.Linear(hidden_size, out_dim), nn.Softmax()
        )
