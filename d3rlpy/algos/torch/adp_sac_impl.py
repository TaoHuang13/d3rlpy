...
class AdpSACImpl(DDPGBaseImpl):
    ...
    def compute_actor_loss(self, batch: TorchMiniBatch) -> torch.Tensor:
        ...
        # mixed_log_prob = (prior_weight * prior_log_prob + (1 - prior_weight) * uniform_log_prob) * self._log_temp().exp() 
    ...
    #         mixed_log_prob = prior_weight * prior_log_prob + (1 - prior_weight) * uniform_log_prob
    #         kl_div.append(policy_log_prob - mixed_log_prob)
    ...
    def compute_prior_weight_loss(self, batch: TorchMiniBatch) -> torch.Tensor:
        ...
        for i in range(1):
            ...
            # prior_weight = self._prior_weight(batch.observations)
            # #mixed_action = prior_weight * prior_action + (1 - prior_weight) * uniform_action
            # q_mix1 = self._q_func(batch.observations, prior_action, "min") * prior_weight
            # q_mixes.append(q_mix1)
            # q_mix2 = self._q_func(batch.observations, uniform_action, "min") * (1 - prior_weight)
            # q_mixes.append(q_mix2)
